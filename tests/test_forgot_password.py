...
class TestPasswordReset(TestCase):
	...
	def test_update_password_reset_used_reset_code(self):
		"""Test a PUT request to '/api/password/reset/'.

		Test a PUT request when the reset object has already been used.
		"""
		url = reverse('password-reset-list')
		# create the reset password request
		data = {
		...
		self.assertEqual(response.status_text, 'OK')
		self.assertTrue('reset successful' in response.data.get('detail'))
		# attempt to use same request code to reset password
		same_url = reverse('password-reset-list') + reset_code + '/'
		data = {
			'new_password': fake.password()
		}
		json_data = json.dumps(data)
		response = self.client.put(
			same_url, json_data, content_type='application/json')
		self.assertEqual(response.status_code, 400)
		self.assertEqual(response.status_text, 'Bad Request')
		self.assertTrue(
			'That password reset request has expired' in
			response.data.get('status')
		)
